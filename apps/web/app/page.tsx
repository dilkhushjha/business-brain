...
function isPositiveSeverity(severity?: string) {
  return ["positive"].includes(normalizeSeverity(severity));
}
...
export default function Home() {
  ...
  function renderSection() {
    ...
    </div>{anomalies.length > 0 && <section className="card anomalyCard"><div className="cardTitle"><b>Exceptions</b><small>Unusual movements worth investigating</small></div>{anomalies.map((a, i) => <div className="anomaly" key={i}><span className={`iconChip sm tone-${isPrioritySeverity(String(a.severity || "")) ? "danger" : "amber"}`}><Icon name="alert" className="icon" /></span><div className="signalBody"><div className="signalTop"><b>{a.name}</b><span className={`tag ${isPrioritySeverity(String(a.severity || "")) ? "danger" : "warning"}`}>{a.severity.toUpperCase()}</span></div><p>Revenue {a.change_pct >= 0 ? "increased" : "decreased"} <strong>{Math.abs(a.change_pct).toFixed(0)}%</strong> versus the prior period.</p></div></div>)}</section>}</section>;
    if (activeSection === "reports") return (
      <section className="contentSection">
        ...
        <div className="card reportCard">
          ...
          <div><b>Detailed reporting</b><p>Review the business data and performance views. Report exports can be added here as the reporting layer grows.</p></div>
        </div>
      </section>
    );
    return (
      <section className="contentSection">
        <SectionTitle title="DATA & IMPORTS" subtitle="Upload, validate and commit business data without leaving the dashboard" />
        <ImportWorkspace businessName={businessName} onImported={() => setDataVersion((v) => v + 1)} onDone={() => setActiveSection("overview")} />
      </section>
    );
  }
