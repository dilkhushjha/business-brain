...
import { apiFetch, ApiAuthError, getBusinessId } from "../../lib/api";
...
export default function ImportPage(){
 const [businessName,setBusinessName]=useState("your business"),[file,setFile]=useState<File|null>(null),[preview,setPreview]=useState<Preview|null>(null),[result,setResult]=useState<ImportResult|null>(null),[error,setError]=useState(""),[busy,setBusy]=useState(false);
 useEffect(()=>{ fetchBusinessName() },[]);
 async function fetchBusinessName(){
   try {
     const r=await apiFetch("/auth/me");
     if(r.ok){const u=await r.json();setBusinessName(u.business?.name||"your business")}
   } catch {}
 }
