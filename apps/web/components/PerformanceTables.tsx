      ...
      {section === "customers" && (concentration || declining.length || inactive.length) && (
        <div className="customerCommandCenter">
          ...
          <div className="customerCommandGrid">
            <section className="customerLeadersSection"><div className="customerCommandHead"><div><span className="eyebrow">REVENUE LEADERS</span><h4>Top customers</h4></div><span>Last 30 days</span></div><div className="topCustomerCards">{customers.length ? customers.map((c,i) => <div className="topCustomerCard" key={c.name}><div className="topCustomerRank">{i+1}</div><div className="topCustomerBody"><b>{c.name}</b><span>{c.orders} orders</span></div><strong>{money(c.revenue)}</strong></div>) : <div className="customerCommandEmpty">No customer sales found in the last 30 days.</div>}</div></section>
