      ...
      {section === "customers" && (concentration || declining.length || inactive.length) && (
        <div className="customerCommandCenter">
          ...
          <div className="customerCommandGrid">
            {concentration && <section className="customerLeadersSection"><div className="customerCommandHead"><div><span className="eyebrow">REVENUE LEADERS</span><h4>Top customers</h4></div><span>Last 30 days</span></div><div className="topCustomerCards">{concentration.top_customers.map((c,i) => <div className="topCustomerCard" key={c.name}><div className="topCustomerRank">{i+1}</div><div className="topCustomerBody"><b>{c.name}</b><span>{c.share_pct.toFixed(0)}% of revenue</span></div><strong>{money(c.revenue)}</strong></div>)}</div></section>}
