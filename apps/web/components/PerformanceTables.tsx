  ...
  return <>
    <section className="grid performanceGrid">
      <div className="card">
        ...
        {products.length ? products.map((p, i) => <div className="performanceRow" key={p.name}>
          <span className="rank">{i + 1}</span>
          <div><b>{p.name}</b><small>{p.quantity} units sold</small></div>
          <strong>{money(p.revenue)}</strong>
        </div>) : <p className="emptyInsight">No product sales in the current 30-day period.</p>}
      </div>
      <div className="card">
        <div className="cardTitle"><h3>Top customers</h3><small>Last 30 days</small></div>
        {customers.length ? customers.map((c, i) => <div className="performanceRow" key={c.name}>
          <span className="rank">{i + 1}</span>
          <div><b>{c.name}</b><small>{c.orders} orders</small></div>
          <strong>{money(c.revenue)}</strong>
        </div>) : <p className="emptyInsight">No customer sales in the current 30-day period.</p>}
      </div>
    </section>
    ...
    {(productConcentration || productMomentum.length) && <>
      ...
      <section className="customerInsightGrid">
        {productConcentration && <div className="card customerInsightCard">
          <div className="cardTitle"><h3>Revenue concentration</h3><small>All imported sales</small></div>
          ...
          {productConcentration.top_products.map((p, i) => <div className="customerMiniRow" key={p.name}>
        ...
        <div className="card customerInsightCard">
          <div className="cardTitle"><h3>Products gaining or losing momentum</h3><small>30-day comparison</small></div>
          {productMomentum.length ? productMomentum.map(p => <div className="customerAlert" key={p.name}>
            <div><b>{p.name}</b><span className={`tag ${p.direction === "up" ? "good" : p.severity === "high" ? "danger" : "warning"}`}>{p.direction === "up" ? "GROWING" : "DECLINING"}</span></div>
          ...
          </div>) : <p className="emptyInsight">No product has crossed the current momentum threshold.</p>}
        </div>

        {productConcentration && <div className="card customerInsightCard">
          <div className="cardTitle"><h3>Product action</h3><small>What the numbers suggest</small></div>
