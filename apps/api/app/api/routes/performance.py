...
def products(business_id: UUID, days: int = 30, limit: int = 10, db: Session = Depends(get_db)):
    return top_products(db, business_id, days=days, limit=limit)


@router.get("/{business_id}/customers")
def customers(business_id: UUID, days: int = 30, limit: int = 10, db: Session = Depends(get_db)):
