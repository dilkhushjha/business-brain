...
def inactive(business_id: UUID, inactive_days: int = 45, limit: int = 10, db: Session = Depends(get_db), _auth: dict = Depends(require_business_access)):
    return inactive_customers(db, business_id, inactive_days=inactive_days, limit=limit)

@router.get("/{business_id}/declining")
def declining(business_id: UUID, days: int = 30, threshold: float = 25, limit: int = 10, db: Session = Depends(get_db), _auth: dict = Depends(require_business_access)):
