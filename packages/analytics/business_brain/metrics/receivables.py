...
def receivables_summary(db: Session, business_id: UUID, as_of: date | None = None) -> dict[str, Any]:
...
def overdue_customers(db: Session,business_id: UUID,limit:int=10,as_of: date | None = None)->list[dict[str,Any]]:
    today = as_of or date.today(); rows=db.execute(select(CustomerModel.name,SaleModel.total_amount,SaleModel.paid_amount,SaleModel.due_date).join(SaleModel,SaleModel.customer_id==CustomerModel.id).where(SaleModel.business_id==business_id,SaleModel.due_date<today)).all(); agg={}
