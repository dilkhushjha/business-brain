...
def upgrade() -> None:
    ...
    if not inspector.has_table("business_brain_connectors"):
        op.create_table(
    ...
    existing_indexes = {index["name"] for index in inspector.get_indexes("business_brain_connectors")}
    if "ix_business_brain_connectors_business_id" not in existing_indexes:
        op.create_index("ix_business_brain_connectors_business_id", "business_brain_connectors", ["business_id"])
    if "ix_business_brain_connectors_status" not in existing_indexes:
...
def downgrade() -> None:
    ...
    existing_indexes = {index["name"] for index in inspector.get_indexes("business_brain_connectors")}
    if "ix_business_brain_connectors_status" in existing_indexes:
        op.drop_index("ix_business_brain_connectors_status", table_name="business_brain_connectors")
    if "ix_business_brain_connectors_business_id" in existing_indexes:
